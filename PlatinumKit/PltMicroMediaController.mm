...
PLT_MicroMediaController::OnMRAdded(PLT_DeviceDataReference& device)
{
    ...
    if (NPT_SUCCEEDED(device->FindServiceByType("urn:schemas-upnp-org:service:AVTransport:*", service))) {
        ...
        //根据名字选择上次的device
        NSString *redername = [[NSUserDefaults standardUserDefaults] objectForKey:@"renderName"];
        NPT_String name = device->GetFriendlyName();
        NSString *temp = [NSString stringWithCString:name.GetChars() encoding:NSUTF8StringEncoding];
...
//对应获取DMS
int PLT_MicroMediaController::getDMS(){
//    HandleCmd_getms();
...
int PLT_MicroMediaController::getDMR() {
...
const char * PLT_MicroMediaController::getCurrentDMR(){
    PLT_DeviceDataReference device;
    GetCurMediaRenderer(device);
    if (!device.IsNull()) {
        printf("Current media renderer: %s\n", (const char*)device->GetFriendlyName());
...
void PLT_MicroMediaController::OnGetVolumeResult(
                                                 ...
                                                 void*                     userdata)
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"OnGetVolumeResult" object:[NSNumber numberWithInt:volume]];
}

void PLT_MicroMediaController::OnGetMediaInfoResult(NPT_Result res, PLT_DeviceDataReference &device, PLT_MediaInfo *info, void *userdata)
{
    [[DLNADataManager DefaultDataManager] setMediaInfo:info];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"OnGetMediaInfoResult" object:nil];
}

void PLT_MicroMediaController::OnGetPositionInfoResult(NPT_Result res, PLT_DeviceDataReference &device, PLT_PositionInfo *info, void *userdata)
{
    [[DLNADataManager DefaultDataManager] setPositionInfo:info];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"OnGetPositionInfoResult" object:nil];
}
